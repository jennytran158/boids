...
#include <boids/plugins/metrics/SOSMetrics/SOSMetrics.h>
...
REGISTER_PLUGIN(scrimmage::Metrics, scrimmage::metrics::SOSMetrics,
                SOSMetrics_plugin)
...
void SOSMetrics::init(std::map<std::string, std::string> &params)
{
    params_ = params;
    ...
    auto killed_cb = [&](scrimmage::MessagePtr<int> msg) {
        scores_[msg->data].increment_killed(); //but why
    };
    subscribe<int>("GlobalNetwork", "Killed_IDs", killed_cb);
}

bool SOSMetrics::step_metrics(double t, double dt)
{
    if (!initialized_)
    {
...
void SOSMetrics::calc_team_scores()
{
    ...
    for (auto &kv : scores_)
    {
        ...
        team_coll_scores_[team_id].add_killed(score.killed());
    }
    ...
    for (auto &kv : team_coll_scores_)
    {
        ...
        team_metrics_[team_id]["killed"] = score.killed();
    ...
    headers_.push_back("killed");
}

void SOSMetrics::print_team_summaries()
{
    for (std::map<int, SOSScore>::iterator it =
         ...
         it != team_coll_scores_.end(); ++it)
    {
        ...
        cout << "Team ID: " << it->first << (it->first == 1 ? " (Attackers)" : " (Defenders)");
        if (survived)
        {
        ...
        cout << "Killed: " << it->second.killed() << endl;
